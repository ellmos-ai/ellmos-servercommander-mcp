...
DEFAULT_SCHEMA_DESCRIPTIONS: dict[str, str] = {
    ...
    "dns_hosts": "Host names to resolve through DNS.",
    ...
    "persist_report": "Persist the analysis summary as a JSON report.",
    "ping_hosts": "Hosts to probe with a single ping.",
    "profile": "Deployment profile name.",
    "query": "Search query.",
    "record_history": "Persist this dry-run deployment plan in the local history database.",
    "report_name": "Optional report filename stem.",
    "remote_path": "Remote target path.",
    "subject": "Email subject.",
    "storage_paths": "Local paths whose storage capacity should be checked.",
...
SCHEMA_TRANSLATIONS: dict[str, dict[str, str]] = {
    "de": {
        ...
        "dns_hosts": "Hostnamen, die per DNS aufgelöst werden sollen.",
        ...
        "persist_report": "Analysezusammenfassung als JSON-Report speichern.",
        "ping_hosts": "Hosts, die mit einem einzelnen Ping geprüft werden sollen.",
        "profile": "Name des Deployment-Profils.",
        "query": "Suchanfrage.",
        "record_history": "Diesen Deployment-Dry-run in der lokalen History-Datenbank speichern.",
        "report_name": "Optionaler Dateiname ohne Endung für den Report.",
        "remote_path": "Entfernter Zielpfad.",
        "subject": "E-Mail-Betreff.",
        "storage_paths": "Lokale Pfade, deren Speicherkapazität geprüft werden soll.",
    ...
    "es": {
        ...
        "dns_hosts": "Nombres de host que se resolverán mediante DNS.",
        ...
        "persist_report": "Guardar el resumen del análisis como informe JSON.",
        "ping_hosts": "Hosts que se comprobarán con un único ping.",
        "profile": "Nombre del perfil de despliegue.",
        "query": "Consulta de búsqueda.",
        "record_history": "Guardar este dry-run de despliegue en la base de historial local.",
        "report_name": "Nombre base opcional del informe.",
        "remote_path": "Ruta remota de destino.",
        "subject": "Asunto del correo.",
        "storage_paths": "Rutas locales cuya capacidad de almacenamiento se debe comprobar.",
    ...
    "zh": {
        ...
        "log_text": "内联访问日志文本。",
        "message_id": "消息标识符。",
        "persist_report": "将分析摘要保存为 JSON 报告。",
        "ping_hosts": "要使用单次 ping 探测的主机。",
        "profile": "部署配置名称。",
        "query": "搜索查询。",
        "record_history": "将此次部署 dry-run 保存到本地历史数据库。",
        "report_name": "可选的报告文件名主体。",
        "remote_path": "远程目标路径。",
        "subject": "邮件主题。",
        "storage_paths": "要检查存储容量的本地路径。",
        "timeout": "请求超时时间（秒）。",
        "to": "邮件收件人。",
    ...
    "ja": {
        ...
        "message_id": "メッセージ識別子。",
        "persist_report": "分析サマリーを JSON レポートとして保存します。",
        "ping_hosts": "1 回の ping で確認するホスト。",
        "profile": "デプロイプロファイル名。",
        "query": "検索クエリ。",
        "record_history": "このデプロイ dry-run をローカル履歴データベースに保存します。",
        "report_name": "任意のレポートファイル名のベース。",
        "remote_path": "リモートの宛先パス。",
        "subject": "メール件名。",
        "storage_paths": "容量を確認するローカルパス。",
    ...
    "ru": {
        ...
        "dns_hosts": "Имена хостов для разрешения через DNS.",
        ...
        "persist_report": "Сохранить сводку анализа как JSON-отчет.",
        "ping_hosts": "Хосты для проверки одним ping-запросом.",
        "profile": "Название профиля деплоя.",
        "query": "Поисковый запрос.",
        "record_history": "Сохранить этот dry-run деплоя в локальную базу истории.",
        "report_name": "Необязательная основа имени файла отчета.",
        "remote_path": "Удаленный целевой путь.",
        "subject": "Тема письма.",
        "storage_paths": "Локальные пути для проверки доступного места.",
...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "de": {
        ...
        "tool.sc_host_diagnostics": "Prüft konfigurierte lokale Speicher-, Ping- und DNS-Diagnosen.",
    ...
    "es": {
        ...
        "tool.sc_host_diagnostics": "Ejecuta diagnósticos configurados de almacenamiento local, ping y DNS.",
    ...
    "zh": {
        ...
        "tool.sc_host_diagnostics": "运行已配置的本地存储、ping 和 DNS 诊断。",
    ...
    "ja": {
        ...
        "tool.sc_host_diagnostics": "設定済みのローカルストレージ、ping、DNS 診断を実行します。",
    ...
    "ru": {
        ...
        "tool.sc_host_diagnostics": "Выполняет настроенную локальную диагностику хранилища, ping и DNS.",
